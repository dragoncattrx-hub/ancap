  ...
  return (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4 hover:border-[var(--accent)]/50 transition-colors">
      <div className="flex items-center gap-2 mb-2">
        ...
        {item.status && (
          <span className="text-xs opacity-50">{item.status}</span>
        )}
      </div>
      <h3 className="font-semibold mb-1">{item.title}</h3>
      {item.description && (
        <p className="text-sm opacity-60 line-clamp-2 mb-2">{item.description}</p>
      )}
      {item.category && (
        <div className="text-xs opacity-40">{item.category}</div>
      )}
      {item.created_at && (
...
export default function SearchPage() {
  ...
  const doSearch = useCallback(async (q: string, t: string) => {
    if (!q.trim()) { setResults([]); setTotal(0); setByType({}); setSearched(false); return; }
    setLoading(true);
    try {
  ...
  useEffect(() => {
    if (initialQ) void doSearch(initialQ, type);
  }, []);
      ...
      <main className="relative z-10 max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {TYPE_OPTIONS.map((opt) => (
            <button
              key={opt.value}
              onClick={() => { setType(opt.value); if (query.trim()) void doSearch(query, opt.value); }}
        ...
        {loading && (
          <div className="text-center py-12 opacity-60">Searching...</div>
        )}
