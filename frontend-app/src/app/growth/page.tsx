...
export default function GrowthDashboardPage() {
  ...
  async function load() {
    try {
      ...
      setError(e.message || t("growthPage.errLoad"));
    }
  }
      ...
      <main className="container" style={{ paddingTop: 24, paddingBottom: 24 }}>
        <div className="card">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
            <h1 style={{ marginTop: 0 }}>{t("growthPage.title")}</h1>
            <select className="input" value={days} onChange={(e) => setDays(parseInt(e.target.value, 10))} style={{ width: 140 }}>
              <option value={7}>{t("growthPage.days7")}</option>
              <option value={14}>{t("growthPage.days14")}</option>
              <option value={30}>{t("growthPage.days30")}</option>
            </select>
          </div>
