...
export const translations = {
  ...
  ru: {
    ...
    acpLanding: {
      ...
      anchorsCard:
        "Хэши запусков и артефактов записываются в сеть, когда CHAIN_ANCHOR_DRIVER=acp, а ACP_RPC_URL указывает на https://acp1.ancap.cloud/rpc; mock-драйвер остается для локальной разработки.",
    ...
    auth: {
      ...
      switchToBnb: "Переключить на BNB Chain",
      ...
      walletSignInDesc: "Подключи EVM-кошелёк, подпиши login-сообщение и продолжай в ANCAP через wallet-based аутентификацию.",
  ...
  uk: {
    ...
    acpLanding: {
      ...
      anchorsCard:
        "Хеші запусків і артефактів записуються в мережу, коли CHAIN_ANCHOR_DRIVER=acp, а ACP_RPC_URL вказує на https://acp1.ancap.cloud/rpc; mock-драйвер залишається для локальної розробки.",
    ...
    auth: {
      ...
      switchToBnb: "Перемкнути на BNB Chain",
