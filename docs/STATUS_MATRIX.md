...
| Public `ancap-docs` split | In progress / repo prep hardened | The public docs repo does not exist yet, but the seed bundle and repeatable export path are now prepared in-repo via `docs/ANCAP_DOCS_SPLIT.md`, `docs/ANCAP_DOCS_REPO_BOOTSTRAP.md`, `docs/ANCAP_DOCS_LABEL_SEED.md`, `docs/ANCAP_DOCS_DISCUSSIONS_SEED.md`, `.github/CODEOWNERS`, and `scripts/export_ancap_docs.py`; the export now ships a docs-focused root README, includes public-safe GitHub issue/PR templates plus a baseline CODEOWNERS review-routing seed, carries the future repo bootstrap/settings/labels/Discussions checklist plus reusable label and Discussions seeds, rewrites out-of-bundle links to the source monorepo, and validates the bundle for broken relative links before export, while Backend CI reruns the export/public-trust regression slice when those bundle inputs change and external org/repo creation is still pending. |
