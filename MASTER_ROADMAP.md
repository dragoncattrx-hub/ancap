...
- **Deploy**: build `d86b007` deployed to ancap.cloud via `scripts/deploy-ancap-cloud.sh` (no-cache rebuild, `alembic upgrade head` ran migration `c4d5e6f7a8b9`, health/ready/frontend-build checks green); `/internal/frontend-build` serves `d86b007` through Cloudflare.
- **CI/CD + ops**: GitHub Actions secrets configured (`ANCAP_DEPLOY_HOST/USER/SSH_KEY` with a dedicated ed25519 deploy key, `ANCAP_SYSTEM_JOBS_TICK_URL`, `ANCAP_CRON_SECRET`), so push-to-master auto-deploy and the 5-minute jobs-tick scheduler are live (first dispatch run green); `acp1.ancap.cloud/rpc` fixed — host nginx pointed at dead `:18080`, now proxies to the compose proxy on `:8080` (JSON-RPC answers again); `admin@ancap.cloud` mailbox password rotated (stored in server-side `Sicret/`), SPF/DKIM/DMARC records verified present; PTR record still requires a VPS-provider ticket.
...
11. [~] AI Payment Scanner MVP:
   ...
   - [ ] OCR for receipts, invoices, payment screens, and payment documents
