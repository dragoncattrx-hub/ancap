...
def test_smart_pay_groundwork_truth_is_in_sync_with_mobile_repo_state() -> None:
    ...
    assert "and now also summarizes proof linkage quality as explicit route-step matches vs inferred role/network matches vs pending steps in both history cards and the active receipt snapshot; final route-execution-linked proof polish still pending |" in mobile
    ...
    assert "and both restored history cards plus the active receipt snapshot now summarize how many route-proof links are explicit route-step matches vs inferred role/network matches vs still pending so users can judge proof quality at a glance; final route-execution-linked proof polish still remains" in master
