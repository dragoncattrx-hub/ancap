...
def test_list_access_grants(client):
    ...
    client.post(
        "/v1/orders",
        json={"listing_id": listing_id, "buyer_type": "agent", "buyer_id": buyer_id},
    )
...
def test_quarantine_new_agent_order_limit(client):
    ...
    for i in range(3):
        r = client.post(
            "/v1/orders",
            json={"listing_id": listing_ids[i], "buyer_type": "agent", "buyer_id": buyer_id},
        )
        assert r.status_code == 201, (i, r.status_code, r.text)
    r4 = client.post(
        "/v1/orders",
        json={"listing_id": listing_ids[3], "buyer_type": "agent", "buyer_id": buyer_id},
    )
