...
/// @title SACP — Stable ACP (18 decimals), mint/burn only via gateway.
/// @notice USD-targeted, ACP-collateralized commerce stablecoin (see docs/STABLECOIN_SACP_SPEC.md).
///         Distinct from WACP (1:1 ACP wrap). Not a guaranteed fiat redemption instrument.
contract SACP {
    string public constant name = "Stable ACP";
    string public constant symbol = "sACP";
    ...
    /// @notice Mint sACP to `to`. Callable only by the gateway.
    ...
    /// @notice Burn sACP from gateway balance (gateway must hold tokens).
