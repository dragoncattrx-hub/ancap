...
3. Production deploy (run on host):
   ...
   - Use `docs/PRODUCTION_SECRET_BASELINE.md` as the operator-side checklist for where those values must live, what evidence to capture, and what still has to be completed outside the repo before treating production-secret hardening as closed.
